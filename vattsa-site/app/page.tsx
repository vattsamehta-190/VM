...
const Section = ({ id, title, children }: { id: string; title: string; children: React.ReactNode }) => (
  <section id={id} className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <motion.h2 initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.4 }}
      ...
      <main id="home" className="relative overflow-hidden">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
          <div id="journal" className="mb-8 flex justify-center">
            <Button asChild variant="outline" className="w-full sm:w-auto min-w-[400px] max-w-[500px] rounded-2xl px-8 py-6 bg-white text-neutral-900 border border-neutral-300 hover:bg-neutral-50 dark:bg-neutral-900 dark:text-neutral-100 dark:border-neutral-700">
              <a href={SITE.socials.notionJournal} target="_blank" rel="noreferrer" className="flex flex-col items-center text-center w-full leading-snug">
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }}
            ...
            <div className="inline-flex items-start gap-2 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 px-3 py-2 text-xs sm:text-sm">
              ...
              <div className="leading-tight">
                <div>Currently adventuring around <span className="font-medium">{SITE.currentLocation}</span></div>
                <div className="text-[10px] sm:text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">Last updated {lastUpdatedText}.</div>
              </div>
            </div>
          </motion.div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3 justify-center">
            <Button asChild className="rounded-2xl px-4">
              <a href="/projects" className="inline-flex items-center gap-2">See my work <ChevronRight className="h-4 w-4" /></a>
            </Button>
            <Button variant="outline" asChild className="rounded-2xl px-4">
              <a href={`mailto:${SITE.email}`} className="inline-flex items-center gap-2"><Mail className="h-4 w-4" /> Contact</a>
            </Button>
            <Button variant="ghost" asChild className="rounded-2xl px-3">
              <a href={SITE.socials.instagram} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2"><Instagram className="h-4 w-4" /> Instagram</a>
            </Button>
            <Button variant="ghost" asChild className="rounded-2xl px-3">
              <a href={SITE.socials.linkedin} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2"><Linkedin className="h-4 w-4" /> LinkedIn</a>
            </Button>
          </div>
        </div>
      </main>
            ...
            <Card className="rounded-2xl border-neutral-200 dark:border-neutral-800">
              <CardContent className="p-6">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <a href={SITE.ggr.site} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm underline underline-offset-4">Website <ExternalLink className="h-3.5 w-3.5" /></a>
                  <a href={SITE.ggr.spotify} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm underline underline-offset-4">Spotify <ExternalLink className="h-3.5 w-3.5" /></a>
                  <a href={SITE.ggr.apple} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-sm underline underline-offset-4">Apple Podcasts <ExternalLink className="h-3.5 w-3.5" /></a>
                </div>
              </CardContent>
            </Card>
          </motion.div>
